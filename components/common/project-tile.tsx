...
const ProjectTile = ({
  ...
  const isInternalLink = project.url && project.url.startsWith("/");

  const linkClassName = "link overflow-hidden rounded-3xl snap-start";
  const linkStyle = {
  ...
  return isInternalLink ? (
    <Link href={project.url}>
      <a className={linkClassName} style={linkStyle}>
        {tileContent}
      </a>
    </Link>
  ) : (
